...
var endpoint="posts";var hashNav={'#athletes':'athletes','#tips':'tips','#science':'science','#results':'results'}
var page=1;$(document).ready(function(){setCategory();getPosts();$('#loadMore').click(function(event){event.preventDefault();$.get(endpoint,{'page':page},function(response){appendPosts(response.data);handleStatus(response.status.status);page++;},'json');});$('.post').fancybox({'transitionIn':'none','transitionOut':'none','type':'iframe','maxWidth':820,'maxHeight':500});$(window).bind('hashchange',function(){setCategory();$('.post').removeClass('hide');if(typeof(visibleCategory)!='undefined'){$('.post').hide();$('.post.'+visibleCategory).show();}else{$('.post').show();}
$('#posts').masonry('reload');});});function getPosts(){$.get(endpoint,{'page':page},function(response){displayPosts(response.data);handleStatus(response.status.status);},'json');page++;}
...
function handleStatus(status){ if(status==1){$('#loadMore').show();}else{$('#loadMore').hide();}}
